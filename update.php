<?php
  ...
  if(isset($_SESSION['key']))
  {
    if(@$_GET['q']== 'addquiz' && $_SESSION['key']=='admin') 
    {
      ...
      $wrong = $_POST['wrong'];
      $id=uniqid();
      $q3=mysqli_query($con,"INSERT INTO quiz VALUES  ('$id','$name' , '$true' , '$wrong','$total', NOW())");
  ...
  if(@$_GET['q']== 'quiz' && @$_GET['step']== 2) 
  {
    ...
    if($ans == $ansid)
    {
      ...
      $r++;
      $s=$s+$true;
      $q=mysqli_query($con,"UPDATE `history` SET `score`=$s,`level`=$sn,`true`=$r, date= NOW()  WHERE  email = '$email' AND eid = '$eid'")or die('Error124');
    } 
    else
    {
